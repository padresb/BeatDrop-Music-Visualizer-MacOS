...
void ProjectM::RenderFrame()
{
    ...
    // Preserve the caller's framebuffer bindings so the final blit lands on the
    // host-provided render target instead of always falling back to framebuffer 0.
    GLint callerReadFramebuffer = 0;
    GLint callerDrawFramebuffer = 0;
    glGetIntegerv(GL_READ_FRAMEBUFFER_BINDING, &callerReadFramebuffer);
    ...
    glBindFramebuffer(GL_READ_FRAMEBUFFER, static_cast<GLuint>(callerReadFramebuffer));
