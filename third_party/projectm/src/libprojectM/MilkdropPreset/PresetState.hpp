...
public:
    ...
    BlendableFloat fftAttack{0.5f};
    BlendableFloat fftDecay{0.725f};
    ...
    std::shared_ptr<Renderer::Texture> fftTexture; //!< Shader FFT texture used by get_fft() helpers.
    ...
    std::array<float, libprojectM::Audio::SpectrumSamples> fftSmoothed{}; //!< Smoothed mono FFT for shader lookups.
    std::array<float, libprojectM::Audio::SpectrumSamples> fftPeak{};      //!< Peak-hold mono FFT for shader lookups.
    std::array<int, libprojectM::Audio::SpectrumSamples> fftPeakHold{};    //!< Peak-hold countdown per FFT bin.
