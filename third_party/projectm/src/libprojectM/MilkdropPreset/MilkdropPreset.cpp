...
void MilkdropPreset::RenderFrame(const libprojectM::Audio::FrameAudioData& audioData, const Renderer::RenderContext& renderContext)
{
    ...
    {
        // Blur must be built from the freshly warped frame so composite shaders
        // sample the same image the Windows renderer uses for GetBlur*().
...
void MilkdropPreset::UpdateShaderFftTexture()
{
    if (!m_state.fftTexture || m_state.fftTexture->Empty())
    {
        return;
    }

    constexpr float kNoiseFloor = 0.025f;
    // projectM's spectrum buffers are already normalized relative to the
    // Windows renderer's raw FFT path, so we upload them at full scale here.
    constexpr float kUploadScale = 1.0f;
    constexpr float kPeakDecay = 0.97f;

    const float attack = std::clamp(static_cast<float>(m_state.fftAttack), 0.0f, 1.0f);
    const float decay = std::clamp(static_cast<float>(m_state.fftDecay), 0.0f, 1.0f);
    const float decayFactor = (1.0f - decay) * (1.0f - decay);
    const int peakHoldFrames = std::max(1, static_cast<int>(m_state.renderContext.fps > 1.0f ? m_state.renderContext.fps : 60.0f));
    float rawMax = 0.0f;
    float smoothedMax = 0.0f;

    for (std::size_t bin = 0; bin < libprojectM::Audio::SpectrumSamples; ++bin)
    {
        float mono = (m_state.audioData.spectrumLeft[bin] + m_state.audioData.spectrumRight[bin]) * 0.5f;
        rawMax = std::max(rawMax, mono);
        mono *= kUploadScale;
        mono -= kNoiseFloor;
        ...
        if (bin < 5U)
        {
            const float t = static_cast<float>(bin) / 5.0f;
            const float lowCut = 0.15f + 0.85f * (t * t);
            mono *= lowCut;
        }

        auto& smoothed = m_state.fftSmoothed[bin];
        if (mono > smoothed)
        {
            smoothed += (mono - smoothed) * attack;
        }
        else
        {
            smoothed += (mono - smoothed) * decayFactor;
        }
        smoothedMax = std::max(smoothedMax, smoothed);

        auto& peak = m_state.fftPeak[bin];
        auto& hold = m_state.fftPeakHold[bin];
        if (smoothed >= peak)
        {
            peak = smoothed;
            hold = peakHoldFrames;
...
#ifdef MILKDROP_PRESET_DEBUG
    if (m_state.renderContext.frame < 3)
    {
        std::cerr << "[Preset] Shader FFT raw max=" << rawMax
                  << " smoothed max=" << smoothedMax
                  << " attack=" << attack
                  << " decay=" << decay
                  << std::endl;
    }
    ...
    glBindTexture(GL_TEXTURE_2D, m_state.fftTexture->TextureID());
    glTexSubImage2D(GL_TEXTURE_2D,
                    0,
                    0,
                    0,
                    libprojectM::Audio::SpectrumSamples,
                    1,
                    GL_RED,
                    GL_FLOAT,
                    m_state.fftSmoothed.data());
    glTexSubImage2D(GL_TEXTURE_2D,
                    0,
                    0,
                    1,
...
void MilkdropPreset::InitializePreset(PresetFileParser& parsedFile)
{
    ...
    m_state.fftTexture = std::make_shared<Renderer::Texture>(
        "fft",
        libprojectM::Audio::SpectrumSamples,
        2,
        GL_R32F,
